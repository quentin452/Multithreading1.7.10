package fr.iamacat.optimizationsandtweaks.utils.fastrandom;
 ...
 *
 * <p>
 * More details can be found in my paper &ldquo;<a href="http://vigna.di.unimi.it/papers.php#VigFSMXG">Further
 * scramblings
 ...
 * for parallel computations. This class provides also a {@link #split()} method to support recursive parallel
 * computations, in the spirit of
 * Java 8's
...
public class XorShift128PlusRandom extends Random {
    ...
    public void jump() {
        long s0 = 0;
        long s1 = 0;
        for (long l : JUMP) for (int b = 0; b < 64; b++) {
            if ((l & 1L << b) != 0) {
                s0 ^= this.s0;
                s1 ^= this.s1;
            }
