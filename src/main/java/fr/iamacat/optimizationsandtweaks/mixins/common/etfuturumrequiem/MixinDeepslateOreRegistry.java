...
public class MixinDeepslateOreRegistry {
    ...
    /**
     * @author quentin452
     * @reason Fix java.lang.NullPointerException: Initializing game from the init method of DeepslateOreRegistry from Et Futurum Requiem with "bad" mods installed
     */
    @Overwrite
    ...
    @Unique
    private static void optimizationsAndTweaks$processEntry(Entry<RegistryMapping<Block>, RegistryMapping<Block>> entry) {
        ...
        ItemStack stackNorm = optimizationsAndTweaks$getItemStack(oreNorm, entry.getKey().getMeta());
        ItemStack stackDeep = optimizationsAndTweaks$getItemStack(oreDeep, entry.getValue().getMeta());
        optimizationsAndTweaks$addSmeltingRecipeIfNeeded(stackNorm, stackDeep);
    }

    @Unique
    private static Block optimizationsAndTweaks$getOreNorm(Entry<RegistryMapping<Block>, RegistryMapping<Block>> entry) {
    ...
    @Unique
    private static Block optimizationsAndTweaks$getOreDeep(Entry<RegistryMapping<Block>, RegistryMapping<Block>> entry) {
    ...
    private static boolean optimizationsAndTweaks$addSmeltingRecipeIfNeeded(ItemStack stackNorm, ItemStack stackDeep) {
        ...
        try {
            ItemStack smeltingResult = FurnaceRecipes.smelting().getSmeltingResult(stackNorm);
            if (smeltingResult != null) {
                GameRegistry.addSmelting(stackDeep, smeltingResult, FurnaceRecipes.smelting().func_151398_b(stackNorm));
            } else {
